...
-- ============================================================
-- MIGRATION 005 — TEACHING FEATURES
-- Adds dialogue audio, AI-generated questions, and quiz attempts.
-- Run after schema.sql and 004_add_students.sql have been applied.
-- ============================================================

-- ── Extend books table ──────────────────────────────────────
-- dialogue_audio_path: S3 key for the two-voice teaching dialogue MP3.
-- dialogue_status:     mirrors book_status — tracks dialogue generation
--                      independently of standard audio generation.
--                      A book can have standard audio ready while
--                      dialogue is still processing, and vice versa.

ALTER TABLE books
    ADD COLUMN dialogue_audio_path TEXT,
    ADD COLUMN dialogue_status     book_status NOT NULL DEFAULT 'pending';


-- ── Questions ───────────────────────────────────────────────
-- Stores AI-generated multiple choice questions per book.
-- Generated by Groq during the processor pipeline and synthesised
-- to audio.
...
CREATE TABLE questions (
    ...
    options             JSONB       NOT NULL,-- Four options stored as a JSONB array: ["option1","option2","option3","option4"],JSONB allows flexible querying while keeping the schema simple.
    correct_index       INTEGER     NOT NULL CHECK (correct_index BETWEEN 0 AND 3),-- JSONB allows flexible querying while keeping the schema simple.
    question_audio_path TEXT,
    audio_status        book_status NOT NULL DEFAULT 'pending',
    order_index         INTEGER     NOT NULL DEFAULT 0,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Primary access pattern: fetch all questions for a book's quiz.
CREATE INDEX idx_questions_book_id ON questions(book_id);


-- ── Quiz attempts ────────────────────────────────────────────
-- Records each time a grade completes a quiz for a book.
-- Multiple attempts are allowed and all are stored — the teacher
-- can see improvement over time, not just the latest score.
-- No student-level tracking — grades share a single account so
-- attempts are attributed to the grade as a whole.

CREATE TABLE quiz_attempts (
    id           UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    book_id      UUID        NOT NULL REFERENCES books(id) ON DELETE CASCADE,
    grade_id     UUID        NOT NULL REFERENCES grades(id) ON DELETE CASCADE,
    score        INTEGER     NOT NULL CHECK (score >= 0),-- score / total allow percentage calculation without storing floats.
    total        INTEGER     NOT NULL CHECK (total > 0),
    answers      JSONB       NOT NULL DEFAULT '[]',-- Individual answers stored as JSONB array of selected indices.
    completed_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Primary access patterns:
-- Teacher views all attempts for a grade across all books.
-- Teacher views all attempts for a specific book.
CREATE INDEX idx_attempts_grade_id   ON quiz_attempts(grade_id);
CREATE INDEX idx_attempts_book_id    ON quiz_attempts(book_id);
CREATE INDEX idx_attempts_completed  ON quiz_attempts(completed_at DESC);